...
class AspectQ(Document):
  ...
  def reviewp(self):
    if self.parent=="true":
      ...
      task= Status.objects(unique_identifier=survey_id[0]+self.provider)
    else:
      survey_id= self.survey_id
      task= Status.objects(unique_identifier=survey_id+self.provider)
    print ("task",task)
    try:
      status= task.scraped_status
      ...
      print ("ReviewP Exception Occured ***",e,"*** for",self.survey_id,self.provider)
  ...
  def scrap(self):
    if self.parent=="true":
      ...
      try:
        task=Status(unique_identifier=self.survey_id+self.provider).count()
        if task==0:
          Status(unique_identifier= self.survey_id+self.provider,scraped_status="success").save()
        else:pass
      except Exception as e:
        print("Exception",e,"for",self.survey_id)
        try:Status(unique_identifier= self.survey_id+self.provider,scraped_status="success").save()
        except:pass
    else:
