...
=======
		...
		for i in range(1, endvalue):
			new_url = self.url[:marker] + "or" + str(i*10) + "-" + self.url[marker:]
>>>>>>> 0f43868b62ea30e822ceddeb9d5895cab42270f9
	...
	def sub_get(self,link):
		...
		for j in review_link:
			...
			if review_res != None:
				...
				try:
					save = Reviews(review_identifier=review_identifier,
						survey_id=self.sid, datetime=parse_date, date_added=date,
						provider=self.p, review=review, rating=rating, review_link=full_url,
						sentiment=sentiment).save(validate=False)
